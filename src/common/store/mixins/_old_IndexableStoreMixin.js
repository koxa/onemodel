  ...
  add(item, silent, updateIfExists) {
    ...
    if (indexes) {
      ...
      for (; i < len; i++) {
        ...
        if (!value && index.type === modelClass.INDEX_PRIMARY) {
          // if Primary field value is not defined or empty, we will use Id instead
          value = item.getId();
        }
        ...
        if (this._indexedItems[name][value]) {
          if (!updateIfExists) {
            ...
            console.log(
              'item was updated because already exists by index: ' + name + ', value:' + value,
            );
