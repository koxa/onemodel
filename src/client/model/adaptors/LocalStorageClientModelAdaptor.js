...
class LocalStorageAdaptor extends BaseModelAdaptor {
  static create(data, params) {
    ...
    if (!localCollection) {
      ...
      window.localStorage.setItem(
        this.getKey(),
        JSON.stringify(Object.assign(JSON.parse(localCollection), { [data.getClientId()]: data })),
      );
    }
    return this;
  }
  ...
  static read(id, params) {
    ...
    if (json) {
      ...
      const result = [];
      if (id) {
        // if id defined look up model by id immediately
        return obj[id];
      } else {
        // otherwise search by params
        for (let id in obj) {
          let includeModel = true;
          let model = obj[id];
          for (let param in params) {
            // check if every param is in object
