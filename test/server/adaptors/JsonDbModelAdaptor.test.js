...
describe('JsonDbModelAdaptor', () => {
  ...
  describe('insertMany && deleteMany', () => {
    it('should insert and delete multiple documents from the collection', async () => {
      ...
      const insertMany = await TestTableJsonDbModel.insertMany(testData);
      expect(insertMany).toEqual({ insertedCount: 3, insertedIds: [10, 11, 12] });

      const readInsert = await TestTableJsonDbModel.read({
        filter: {
      ...
      const result = await TestTableJsonDbModel.deleteMany(insertMany.insertedIds);
      expect(result.deletedCount).toBe(3);

      const readDelete = await TestTableJsonDbModel.read({
        filter: {
          id: { $in: insertMany.insertedIds },
        },
      });
